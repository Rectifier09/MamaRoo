...
| Token | Hex | Role |
|---|---|---|
| Warm cream | `#FFF3E4` | Primary background. Never pure white. |
| Peach blossom | `#FFB4A2` | Primary brand color — cards, surfaces, backgrounds. |
| Soft coral | `#FF8C7A` | Decorative/accent only — illustration, icons, badges, backgrounds. **Not for text-bearing button fills.** |
| CTA coral | `#C94A3C` | Button fills specifically, paired with white text. |
| Golden sunrise | `#FFD166` | Joy / milestone accents only. **Never used as text color.** |
| Sage mist | `#B8D8BA` | Secondary/wellness accent surfaces. **Never used as raw text color.** |
| Darkened sage | `#4C6B4E` | Use whenever sage needs to appear as legible text. |
| Deep plum | `#6B4256` | Body text and emphasis. Replaces black everywhere. |

### 2.2 Usage rules

- Text on peach, sage, or gold backgrounds must be **18px+ or bold** — never small body copy at those pairings.
...
|---|---|---|---|
| Deep plum on cream | 7.56:1 | 82.2 | ✅ Body text |
| Deep plum on white | 8.27:1 | 88.4 | ✅ Body text |
| White on deep plum | 8.27:1 | 93.1 | ✅ Body text |
| Darkened sage on cream | 5.45:1 | 73.6 | ✅ Body text |
| White on CTA coral `#C94A3C` | 4.64:1 | 76.8 | ✅ Button label |
| Deep plum on peach / sage / gold | 4.85–5.74:1 | 55.8–65.1 | ⚠️ Headings/bold only, 18px+ |
...
- Card shadows: warm-tinted (derived from deep plum, e.g. `rgba(107,66,86,0.08)`), never gray or pure black.
