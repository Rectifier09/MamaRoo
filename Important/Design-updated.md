...
### Background
- Base: warm cream `#F7EEE3`, flat, never pure white.
- Global background wash: a subtle top-to-bottom gradient, `#F7EEE3` → `#F7DFD9` (soft blush) — same hue family as peach, far lower saturation, reads as ambient warmth/depth rather than a color statement. Applies app-wide as the default surface, distinct from the signature glow below.
...
- This glow is the **only moving** background element in the app — no other screen gets ambient motion. The gradient wash is static everywhere; motion stays a single signature moment, not a texture applied everywhere (restraint, §6).

**Edge cases:**
- Both gradient stops keep body-text contrast comfortably above AA at every point in between (plum on cream: 11.24:1 / 87.7 APCA; plum on blush: 10.14:1 / 81.2 APCA) — confirmed safe to place text anywhere on the wash, not just the lighter end.
- The gradient direction is fixed top-to-bottom only — no diagonal, no radial, no multi-stop; one subtle move, not a design statement.
- Sensitive-moment mode (§8) suppresses the gradient back to flat cream, and suppresses the breathing glow entirely — no gradient, no motion, same as no coral/gold/sage, keeping that mode visually stripped-down.
